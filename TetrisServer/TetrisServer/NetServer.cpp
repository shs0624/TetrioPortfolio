...
bool CNetServer::StartNetServer(ULONG ip, LONG port, bool bNagleEnabled, int maxConnection, const unsigned char programKey, const unsigned char fixedKey)
{
	int retval;
...
unsigned int WINAPI CNetServer::IOCPWorkerThread(LPVOID arg)
{
	...
	while (1)
	{
		...
		retval = GetQueuedCompletionStatus(thisPtr->_NetIOCPHandle, &cbTransferred, 
...
void CNetServer::InitializeSessions(ULONG maxConnection)
{
	...
	//_emptyIndexStack = new std::stack<ULONGLONG>();
...
bool CNetServer::Disconnect(ULONGLONG sessionID)
{
	...
	//ptr->bCanceled = true;
	InterlockedExchange((LONG*)&(ptr->bCanceled), true);
